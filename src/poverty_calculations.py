...
def get_population_per_county():
    ...
    '''
    out_dict={}
    f=open('files/PEP_2018_PEPANNRES_with_ann.csv','r',newline='',encoding='latin-1')
    reader=csv.reader(f)
    header1=next(reader)
    header2=next(reader)
    for row in reader:
...
def plot_county_accident_rates(fname):
    ...
    df_pov=pd.read_excel('files/est18all.xlsx',converters={'State FIPS Code':str,'County FIPS Code':str})
